...
- **改了 `.claude/tests/**` 就会让 `FRAMEWORK-MANIFEST.txt` 陈旧，而 test-setup.sh 的全量比对会当场红、还连带把 ⑨ 的 doctor 基线断言拖红**（一个根因两处红）。清单不在 tester 的 Scope 时，证明「只是陈旧不是真坏」的办法是整仓 tar 到 /tmp、在副本上跑 `gen-manifest.sh` 再跑 test-setup.sh——本仓零改动，还能顺带照出被清单红盖住的别的红（我这次就是这样才看见 `session-rules-banner.sh` 那处漏改）。
...
- **改任何被 FRAMEWORK-MANIFEST 登记的 `.claude/` 文件（含 `agent-memory/**/*.md`），`test-setup.sh` 当场红**：清单存的是 LF 归一后的 sha256，得 `.claude/scripts/gen-manifest.sh` 重生成才平，红锁在库期间它会一路红着。归因手法是 `git worktree add --detach /tmp/head-wt HEAD` 在 pristine HEAD 上跑同一个测试对拍（对本仓只读，跑完 `git worktree remove --force`）——实测 HEAD 自己就带一条存量不符（上一轮 reviewer 写了 agent-memory 没重生成清单），不对拍就会把存量红算到自己头上。
