...
[接线点（hook 侧，catalog + node 双满足才启用，否则静默走原逻辑）]
    ...
    - **record-authorship ↔ authorship record（作者台账自动记账）**：PostToolUse(Edit|Write|NotebookEdit) 挂 `record-authorship.sh|.ps1`——每次编辑把「谁写了哪个文件」追加进账本，`review verdict` 才判得了「评审者是不是作者」。**这是记账不是闸**：任何内部失败一律 exit 0 + 一行 stderr（PostToolUse 的非 0 退出码会回灌工具结果），引擎调用套 `timeout 10`（settings 里注册的 `timeout` 兜第二道）；catalog/node/jq 缺任一静默跳过；Fast Mode 放行。
