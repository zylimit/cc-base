...
[回传与验收]
    ...
    - **统一回执信封**，各 Sub-Agent 一律以四态自评开头：**Status**（DONE / DONE_WITH_CONCERNS / NEEDS_CONTEXT / BLOCKED；tester 可用 PASS / FAIL 表示运行器结果）/ **Changed**（实际改动的文件或产物，只读角色写 None）/ **Verified**（实际跑过且拿到结果的核查）/ **Not verified**（没执行或无法证明的，必须列）/ **Business assumptions**（Spec 没写、自己补的判断，没有写 None）/ **Counter-examples**（代码对得上 Spec、Spec 对不上业务的反例，有就报「需求存疑」，主 Agent 回流 product-spec-builder 迭代模式，没有写 None）/ **Domain findings**（本次干活撞出的领域线索——这个领域「事情是怎么算的」的现象，带上你手上的证据：实测结果、字段的真实格式、真库的实际状态、外部系统的实际行为。**报线索不报结论**：现象真、解释未必真，定论要 AI 查外网、人查公司内部各自补齐依据之后才下，不归你；没证据的不写，凭印象凑的口径比没有更糟。本次的范围限制（做完就失效）不算，需求反例走 Counter-examples，对 AI 工作方法的意见走 feedback；主 Agent 裁定收不收、收则派 domain-recorder 写入，没有就写裸 `None`——闸按字面判，`None（本次纯只读）` 这种带括注的算「有发现」，会多换来一句咨询性提醒；宁可这样也不放宽，放宽了会把写在括注里的真发现吞掉）/ **Needs review by** / **Evidence**。
    - **对着 diff 与运行器输出判，不对着实现者的报告判**；翻证据（读 artifact 全文、跑核查三件套）这类体力活可外包，「通过 / 不通过」的判断权留主 Agent，需要时再派 fresh 实例回溯原文核实。
    - **新建文件的改动证明不许用 `git diff`**：对本 session 新建、尚未入库的文件，`git diff` 给的是空输出 + rc 0，与「这文件没被改过」一模一样；改用 `wc -l` + 内容锚点 + `git status` 认 `??`。2026-09-11 实测撞出来的——当天多个派单写着「贴 diff 证明只动了这几个文件」，对新建文件全是看着像通过的空结果。
