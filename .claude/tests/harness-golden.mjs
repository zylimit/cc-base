...
//           argument as <NODE>. Recorded verbatim it cost: every edit to a lib file, a
//           comment included, became a golden diff and forced a re-record. What the binding
//           actually does -- STALE on engine-moved, legacy receipts without the field still
//           matching -- is asserted in test-release-binding.sh, which compares states, not
//           digests.
...
// asserts anything about THIS checkout. S5b closes that hole with a small set of in-repo
...
// every bit of that: a changed byte still fails --check, it just
...
// A sample of the differing field paths, not all of them: a change that moves the whole
// matrix produces thousands. The full count travels beside it as diffCount, so the sample
// can never read as the whole story.
const DIFF_SAMPLE = 5;
...
function checkMatrix() {
  ...
  const sayErr = line => console.error(line);
  ...
  const label = SCENARIOS.length + ' scenarios + in-repo';
  out.assertions = assertions;
  say('');
  if (failed === 0) {
...
const MODES = ['--record', '--check', '--probe'];
const USAGE = 'usage: node .claude/tests/harness-golden.mjs '
  + MODES.join('|') + ' [--strict]';

/**
 * Hand-rolled, because a one-flag grammar is not worth a dependency in a file whose whole
 * point is that it has none. One decision worth stating: an unrecognized trailing argument
 * is an error rather than something to ignore. The old parser took the first non---strict
 * token as the mode and dropped the rest, so a typo'd flag ran the full check and reported
 * success for a run nobody asked for.
 */
function parseArgs(argv) {
  const out = { mode: null, strict: false, error: null };
  for (const a of argv) {
