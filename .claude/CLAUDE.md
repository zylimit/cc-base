...
[总体规则]
    ...
    - **派静默 subagent / 长后台任务前先告知用户**：派 Sub-Agent 或长后台任务前必先一句话告知（静默运行 / 预计耗时 / 完成会通知），别让用户对着无输出干等误判卡死（预告这一下靠自觉，完成侧才有 `notify.mjs` 兜底）。确认挂死迹象（CPU 0% 而时长仍涨 / 已定位根因却没有进展）后立即报告并止损，不许「进程还活着」式观望——观望是最贵的那个选项。工具调用被用户消息中断是 harness 机制信号、≠用户否决方案——有新指示就照办、只是提醒就解释并重发同一方案、不确定先问，不擅自切换；禁甩锅。细则见 feedback/subagent-silence-preannounce-interrupt-not-rejection-no-blameshift.md。
...
[开发测试规则]
    ...
    - **闸靠数据留，不靠感觉留**：某闸长期全过 / 全绿、从没产出过 FIX_REQUIRED 或红，就简化或删掉，别为"感觉安全"养无效成本；加闸要能说出它挡住过什么——`gate-audit.sh` 报的就是这个。「验证验证者」那类机制（突变矩阵、尺子的尺子）默认不进发版链与高频路径，只在基建本身变更时手动或低频跑；清理现有机制除了看拦过什么，也按「它判的是代码对不对，还是判我们的验证手段对不对」筛一遍。细则见 feedback/gates-need-empirical-validation.md。
