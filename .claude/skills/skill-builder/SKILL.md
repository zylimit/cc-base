...
[第一性原则]
    ...
    **参考采用看证据**：Skill 带 references 时，主文件写清每份参考的读取时机；判断路由有没有生效，逐场景记「触发事实 → 实际读取的路径 → 参考里哪个判断影响了最终决定」。链接能解析只证明可达，被打开只证明读过，读过却没影响决定不算方法生效，三项缺哪项就留哪项未知。漏读先核对入口的触发条件写没写对、同一规则的定义 / 例外 / 继续条件是否放得够近，再决定要不要把正文挪进主文件；所有分支都要知道的含义留在入口，特定分支的细节留在按条件读取的参考。反过来也查一处清楚的小改动，看是否无故读了专业材料或启动访谈。
...
[工作流程]
    ...
    [第五步：创建文件]
        ...
        - frontmatter 只有 name 和 description？
        - 风格和参照的现有 Skill 一致？
        - 有 references 时，每份都有读取时机？能实际演练的，挑一个该读的场景和一个不该读的小改动，各记一轮「触发事实 → 实际读取路径 → 由此作出的决定」；只看到读取、指不出影响了哪个决定的，只记读取
