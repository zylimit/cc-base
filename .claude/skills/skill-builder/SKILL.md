...
[创建规范]
    ...
    [Section 分类]
        **必须有**（所有 Skill 都有）：
        ...
        - [依赖检测] — 启动时检查前置条件
        - [第一性原则] — 3-5 条核心原则
        - [文件结构] — 只在 skill 目录有模板 / 脚本时写；只有 SKILL.md 一个文件的不画树
        - [初始化] — 入口点

        **推荐有**（大多数 Skill 有）：
        - [对话示例] / 反例 / 收敛条件 / 交接 — 对话型 skill 必有：真实风格的一段对话、AI 跑偏的反例、什么时候停、把什么交给下一阶段。不写 [输出风格]——语气统一在 CLAUDE.md [角色]
...
[工作流程]
    ...
    [第四步：填充内容]
        ...
        - [任务] — 一句话，如有多种模式分别说明
        - [依赖检测] — 列出必需和可选依赖
        - [第一性原则] — 3-5 条，只写本阶段特有的判断依据；CLAUDE.md 已有的铁律（联网优先 / 证据为王 / 写测独立）不重复
