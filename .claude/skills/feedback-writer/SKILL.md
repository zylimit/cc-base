...
[写入流程]
    ...
    2. 检查是否已有同主题 feedback（去重）——已有 → 更新内容 + 更新 updated，并在正文补这次的触发场景（同一条踩第二次，值钱的是第二次长什么样，不是计数器 +1）；与旧条冲突 → 新条 supersedes 写旧文件名，旧条 updated 加一行「被 <新文件> 修正」；没有 → 创建新文件 + 更新索引
...
[返回格式]
执行完毕后返回给主 Agent：
- 有新记录："记录了 1 条 feedback：[标题]（[文件名]）；落点建议：[哪个 skill / rule 的哪一段该改，或「当前产物已够，不动本体」]"
- 更新已有："更新了 [文件名]，补了这次的触发场景；落点建议：[同上]"
- applied_to 为 pending 时在摘要里点明："落地 pending——主 Agent 尚未把纠正应用到当前产物"
- 无信号："无新 feedback"
