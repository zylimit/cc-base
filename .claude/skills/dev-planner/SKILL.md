...
[第一性原则]
    ...
    **价值与未知先行原则**：Phase 顺序由两件事定——先交付核心价值（去掉它产品不成立的那条流程），先验证关键未知（Spec 里的 `[推断]` `[待定]`、没用过的技术、没试过的集成）。基础设施只做支撑第一条价值流程所需的最小部分，不为「地基」单独排一个什么都看不到的 Phase。被依赖方先做仍成立，但依赖是校正项，不是排序主轴。

    **联网优先**：技术栈与关键依赖的版本、兼容性、breaking changes 先 WebSearch 再定（CLAUDE.md 总体规则）。
...
[分析策略]
    **依赖图构建法**
    ...
    3. 构建有向无环图（DAG）
    4. 拓扑排序得到的是「不能违反的先后」，不是 Phase 顺序本身
    骨架只到能跑通第一条价值流程为止，其余基础设施放进用到它的那个 Phase。

    **洋葱剥皮法（由内到外）——排序主轴**
    先按"用户感知价值"排序，再用依赖图校正（只在被依赖方必须先做时调整）：
    ...
    **假设前置法**
    每个 Phase 写明「验证的假设」——来自 Spec 的 `[推断]` / `[待定]` 条目、没用过的框架、关键第三方 API、性能敏感点；没有就写「无」。含核心假设（假设不成立整条流程要重来）的 Phase 排在纯功能堆叠之前，且验收标准里写清「假设成立的证据是什么」：
    - Spec 标 `[推断]` 的核心行为 → 最早能演示它的 Phase 就拿真数据验，验完回 Spec 把标记改成 `[确认]` 或改需求
...
[工作流程（生成模式）]
    ...
    [输出阶段]
        ...
        第三步：自检
            ...
            假设覆盖检查：Spec 的 `[推断]` / `[待定]` 条目每条落在某个 Phase 的「验证的假设」里，或在计划开头写明押后到哪、押后期间按什么做；含核心假设的 Phase 在前
