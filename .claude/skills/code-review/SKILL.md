...
[审查维度清单]
    ...
    把 linter 能抓的问题挡在语义审查之前，别让审查者/人去挑机器该挑的。
        执行： node .claude/hooks/static-check.mjs .   （识栈跑 shellcheck / ruff|py_compile / tsc / node --check）
        - exit 0（全绿）→ 进 Stage 1
    ...
    [功能完整性]
        ...
        - ⚠️ 部分实现 — 缺失的具体内容
        - ❌ 未实现 — Spec 原文引用
        - ❓ 需求存疑 — 代码对得上 Spec，但 Spec 对不上业务（见下 [业务含义核对]）

    [业务含义核对]
        对照派单包 Business Context（这个 Task 为什么做、谁受益、Spec「规则与例外」里的相关条目、progress.md 里适用的规则）再过一遍：
        - Spec 条目与规则 / 例外矛盾（例外那次按这段代码会走错）
        - Spec 条目标着 `[推断]`，而代码现场（数据形态、既有流程、命名）给出了相反证据
        - 代码按 Spec 做了，但受益者拿到的结果对不上「为什么做」
        命中就标「❓ 需求存疑」，写成反例：具体情境 → 按 Spec 会发生什么 → 业务上应发生什么 → 依据（Business Context 哪条 / 现场哪个证据）。
        存疑不算 Stage 1 失败、不阻塞 Stage 2，报告里单列；改需求归 product-spec-builder，审查者不替用户改 Spec。没有 Business Context 的派单，这一段写「无业务上下文，未核」——没核不等于没问题。
...
[工作流程]
    ...
    [第五步：输出审查报告]
         ...
         **❓ 需求存疑（X 项）**
         - [Spec 条目]：情境 → 按 Spec 会… → 业务上应… — 依据：[Business Context 哪条 / 现场证据]
    ...
    - 报告含「❓ 需求存疑」→ 主 Agent 回流 product-spec-builder 迭代模式（反例作输入）；Spec 改了再重审对应条目，没改（用户确认 Spec 没错）则关闭存疑并记进澄清记录
