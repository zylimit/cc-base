...
- 🔒[铁律]✅[已毕业] [三文件同步：随时可 Clear 上下文，靠 recap 完整恢复](three-file-sync-clearable-context-recap-recovery.md) — Product-Spec.md/Product-Spec-CHANGELOG.md/progress.md 即时同步，确保任何时候可 Clear 上下文、靠 recap 完整恢复；决策/约束/完成/需求变更**当下即同步**对应文件（决策进 Decisions 段不混 Done、需求变更 Spec+CHANGELOG 成对更新），不积压不事后补，每个工作单元收尾自检"三文件都同步了吗"；始终贯彻、零马虎
