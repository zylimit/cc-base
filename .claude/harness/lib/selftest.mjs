...
function selftestCases() {
  ...
  return [
    ...
    // S19 spec-lint deferrals -- the new Product-Spec template gives the deferral token a
    // legal home: a row in the pending-questions section, where an open question is the
    // subject matter rather than an unfinished sentence. Outside that section it is still an
    // unfinished document, and inside it a row with a hole defers nothing -- it only records
    // that someone typed the heading. The section name is composed from PLACEHOLDER_TOKENS
    // instead of being hand-escaped, for the ASCII-only reason recorded at the top of this
    // file; the row cells carry no meaning beyond being five non-empty cells.
    ['spec-lint: a deferral inside the pending-questions table is subject matter', () => {
      ...
      const r = lintSpecDoc(doc, 'Product-Spec.md');
      assert.ok(!codesOf(r).includes('PLACEHOLDER'),
    ...
    ['spec-lint: a threshold comparison is not an unfilled template placeholder', () => {
      ...
      assert.deepEqual(budget.findings.filter(f => f.code === 'PLACEHOLDER'), [],
        'a latency budget is a measurement, not a hole someone forgot to fill; got ' + JSON.stringify(budget.findings));
      const slot = lintSpecDoc(specDoc(['- a: b -> c'], { tech: 'product type: <Web / Desktop / CLI>' }), 'f.md');
    ...
    ['spec-lint: a comparison has to look like one on both sides of the span', () => {
      ...
      const shortForm = '<=3 items>';
      const tech = [prompt, 'concurrency < 100 and latency > 1s', 'data volume <10 rows, single table >100 rows', shortForm].join('\n');
      const doc = specDoc(['- a: b -> c'], { tech });
      const want = [at(doc, prompt), at(doc, shortForm)].sort((a, b) => a - b);
    ...
    ['spec-lint: an unfilled double-brace slot is a placeholder, quoted ones are not', () => {
      ...
      const doc = specDoc(['- a: b -> c'], { tech: [bare, quoted].join('\n') });
      const line = doc.split('\n').findIndex(l => l === bare) + 1;
      const ph = lintSpecDoc(doc, 'f.md').findings.filter(f => f.code === 'PLACEHOLDER');
      assert.deepEqual(ph.map(f => f.line), [line],
        'a slot nobody replaced is an unfinished document; the line showing how to write one is not. got ' + JSON.stringify(ph));
    }],
    // One missing back-tick row and everything after it is inside a fence: the rest of the
    // document stops being checked and the file still comes back clean. Silence from a parser
    // that lost its place is the worst kind of green, so the unclosed opener is itself an error.
