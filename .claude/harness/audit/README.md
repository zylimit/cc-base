...
```json
{
  ...
  "entries": [
    { "file": "CLAUDE.md", "line": 12, "rule": "endpoint-override",
      ...
      "context": "<可选：上一行 + 该行 + 下一行、用 \n 连接后的 sha256>",
      "reason": "为什么这条是良性的" }
  ]
}
```

- 条目绑 `{file, line, rule, sha256}`，**行改一个字，豁免自动失效**——白名单没法退化成万能静音。
- **`context` 把绑定从字节扩到语境**。只绑本行只能保证字节没变，保证不了这行还是原来那个意思：
  把一条反例外面的 ``` 围栏删掉，行号、字节、sha256 全不变，而这一行从「**千万别这么写**」
  变成了「照着做」——而 README 举的正当豁免场景恰恰就是「安全文档里的反例」，
  也就是说最常见的用法正是最脆的那种。写了 `context` 就连上下各一行一起绑；
  没写照旧生效，但每次命中都会打一条 `allowlist-entry-not-context-bound` 的 note，
  提醒这条豁免只绑了字节。
...
- **`context` 是可选的，所以默认态绑的只是字节**。没写 `context` 的条目，把豁免行外面的
  ``` 围栏换成空行就能在行号、字节、sha256 全不变的前提下把「**千万别这么写**」翻成「照着做」。
  每次命中会打 `allowlist-entry-not-context-bound` 提醒，但它不拦——想真绑住就写 `context`。
